...
var getDefenders = function (city, callback) {
  async.waterfall([
    ...
    function createSoldier (existingDefenders, protosoldiers, next) {
      var numberAbleToRecruit = Math.floor((new Date() - city.updated_at) / 1000 / 60 * city.army_level ); // 소요시간 / 1분 * 도시의 군사등급
      var soldiersToRecruit = city.soldiers - existingDefenders.length;
      var recruitCount = ( numberAbleToRecruit > soldiersToRecruit ) ? soldiersToRecruit : numberAbleToRecruit;
